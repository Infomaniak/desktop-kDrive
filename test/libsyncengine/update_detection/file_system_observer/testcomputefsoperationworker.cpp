...
void TestComputeFSOperationWorker::testIsLocalTimestampValid() {
    // Collect errors generated by isLocalTimestampValid.
    std::vector<Error> errors;
    _syncPal->setAddErrorCallback([&errors](const Error &error) { errors.push_back(error); });

    const NodeId fileNodeId = "l_ba";
    const SyncPath relativePath = "B/BA";
    ...
    CPPUNIT_ASSERT(
            _syncPal->computeFSOperationsWorker()->isLocalTimestampValid(fileNodeId, NodeType::Directory, -1, relativePath));
    CPPUNIT_ASSERT(_syncPal->computeFSOperationsWorker()->isLocalTimestampValid(fileNodeId, NodeType::Directory,
                                                                                justBeyondOneYear, relativePath));
    CPPUNIT_ASSERT(errors.empty());

    // A file with the current timestamp is valid.
    CPPUNIT_ASSERT(
            _syncPal->computeFSOperationsWorker()->isLocalTimestampValid(fileNodeId, NodeType::File, now, relativePath));
    CPPUNIT_ASSERT(errors.empty());

    // A file with timestamp 0 (Unix epoch) is valid — it is not negative.
    CPPUNIT_ASSERT(
            _syncPal->computeFSOperationsWorker()->isLocalTimestampValid(fileNodeId, NodeType::File, 0, relativePath));
    CPPUNIT_ASSERT(errors.empty());

    // A file with a timestamp just within one year in the future is valid.
    CPPUNIT_ASSERT(_syncPal->computeFSOperationsWorker()->isLocalTimestampValid(fileNodeId, NodeType::File,
                                                                                justWithinOneYear, relativePath));
    CPPUNIT_ASSERT(errors.empty());

    // A file with a negative timestamp is invalid: returns false and an InvalidTimestamp error is reported.
    CPPUNIT_ASSERT(
            !_syncPal->computeFSOperationsWorker()->isLocalTimestampValid(fileNodeId, NodeType::File, -1, relativePath));
    CPPUNIT_ASSERT_EQUAL(size_t{1}, errors.size());
    CPPUNIT_ASSERT_EQUAL(InconsistencyType::InvalidTimestamp, errors.back().inconsistencyType());
    errors.clear();

    // A file with a timestamp beyond one year in the future is invalid: returns false and an InvalidTimestamp error is
    // reported.
    CPPUNIT_ASSERT(!_syncPal->computeFSOperationsWorker()->isLocalTimestampValid(fileNodeId, NodeType::File,
                                                                                 justBeyondOneYear, relativePath));
    CPPUNIT_ASSERT_EQUAL(size_t{1}, errors.size());
