...
void TestJobManager::testWithoutCallback() {
    ...
    for (auto &dirEntry: std::filesystem::directory_iterator(localTmpDir.path())) {
        ...
        const auto job = std::make_shared<UploadJob>(nullptr, driveDbId, dirEntry.path(), dirEntry.path().filename().native(),
                                                     remoteTmpDir.id(), 0, 0);
        JobManager<AbstractJob>::instance()->queueAsyncJob(job);
    ...
    while (!jobIds.empty()) {
        ...
        while (!jobIds.empty() && JobManager<AbstractJob>::instance()->isJobFinished(jobIds.front())) {
            jobIds.pop();
        }
    }
...
void TestJobManager::testWithCallback() {
    ...
    for (auto &dirEntry: std::filesystem::directory_iterator(_localTestDirPath_manyFiles)) {
        ...
        JobManager<AbstractJob>::instance()->queueAsyncJob(job, Poco::Thread::PRIO_NORMAL);
        counter++;
...
void TestJobManager::testCancelJobs() {
    ...
    while ((!JobManager<AbstractJob>::instance()->_data._managedJobs.empty() ||
            !JobManager<AbstractJob>::instance()->_data._queuedJobs.empty() ||
            !JobManager<AbstractJob>::instance()->_data._runningJobs.empty() ||
            !JobManager<AbstractJob>::instance()->_data._pendingJobs.empty()) &&
           (retry > 0)) {
        retry--;
        Utility::msleep(10);
    }

    CPPUNIT_ASSERT(JobManager<AbstractJob>::instance()->_data._managedJobs.empty());
    CPPUNIT_ASSERT(JobManager<AbstractJob>::instance()->_data._queuedJobs.empty());
    CPPUNIT_ASSERT(JobManager<AbstractJob>::instance()->_data._runningJobs.empty());
    CPPUNIT_ASSERT(JobManager<AbstractJob>::instance()->_data._pendingJobs.empty());
...
void TestJobManager::testJobPriority() {
    ...
    JobManager<AbstractJob>::instance()->queueAsyncJob(job1, Poco::Thread::PRIO_LOWEST);
    JobManager<AbstractJob>::instance()->queueAsyncJob(job2, Poco::Thread::PRIO_LOW);
    JobManager<AbstractJob>::instance()->queueAsyncJob(job3, Poco::Thread::PRIO_NORMAL);
    JobManager<AbstractJob>::instance()->queueAsyncJob(job4, Poco::Thread::PRIO_HIGH);
    JobManager<AbstractJob>::instance()->queueAsyncJob(job5, Poco::Thread::PRIO_HIGHEST);
...
void TestJobManager::testJobPriority2() {
    ...
    JobManager<AbstractJob>::instance()->queueAsyncJob(job1, Poco::Thread::PRIO_NORMAL);
    JobManager<AbstractJob>::instance()->queueAsyncJob(job2, Poco::Thread::PRIO_NORMAL);
    JobManager<AbstractJob>::instance()->queueAsyncJob(job3, Poco::Thread::PRIO_NORMAL);
    JobManager<AbstractJob>::instance()->queueAsyncJob(job4, Poco::Thread::PRIO_NORMAL);
    JobManager<AbstractJob>::instance()->queueAsyncJob(job5, Poco::Thread::PRIO_NORMAL);
...
void TestJobManager::testCanRunjob() {
    // Small file jobs
    {
        ...
        for (auto i = 0; i < 20; i++) {
            ...
            CPPUNIT_ASSERT_EQUAL(true, JobManager<AbstractJob>::instance()->canRunJob(job));
            JobManager<AbstractJob>::instance()->queueAsyncJob(job, Poco::Thread::PRIO_NORMAL);
        }
    ...
    // Upload sessions
    {
        ...
        const auto job2 = std::make_shared<DriveUploadSession>(nullptr, driveDbId, nullptr, filepath,
                                                               filepath.filename().native(), remoteTmpDir.id(),
                                                               testhelpers::defaultTime, testhelpers::defaultTime, false, 3);
        CPPUNIT_ASSERT_EQUAL(true, JobManager<AbstractJob>::instance()->canRunJob(job1));
        JobManager<AbstractJob>::instance()->queueAsyncJob(job1, Poco::Thread::PRIO_NORMAL);
        Utility::msleep(200);
        CPPUNIT_ASSERT_EQUAL(false, JobManager<AbstractJob>::instance()->canRunJob(job2));
        while (!JobManager<AbstractJob>::instance()->isJobFinished(job1->jobId())) {
            Utility::msleep(100);
        }
        CPPUNIT_ASSERT_EQUAL(true, JobManager<AbstractJob>::instance()->canRunJob(job2));
    ...
    // Big files download
    {
        ...
        for (auto i = 0; i < 20; i++) {
            ...
            if (!JobManager<AbstractJob>::instance()->canRunJob(job)) {
                noMoreRun = true;
                break;
            }
            JobManager<AbstractJob>::instance()->queueAsyncJob(job, Poco::Thread::PRIO_NORMAL);
...
void TestJobManager::testWithCallbackBigFiles(const SyncPath &dirPath, const uint16_t size, const uint16_t count) {
    ...
    JobManager<AbstractJob>::instance()->setPoolCapacity(4 * (int) std::thread::hardware_concurrency());
