...
class SyncInfoClient : public BaseSync {
    public:
        SyncInfoClient();
        ...
        inline void setStep(const SyncStep step) noexcept { _step = step; }
        inline SyncStep step() const noexcept { return _step; }
        inline void setUnresolvedConflicts(const bool unresolvedConflicts) noexcept {
            _unresolvedConflicts = unresolvedConflicts;
        }
        inline bool unresolvedConflicts() const noexcept { return _unresolvedConflicts; }
        inline void setCurrentFile(const qint64 currentFile) noexcept { _currentFile = currentFile; }
        inline qint64 currentFile() const noexcept { return _currentFile; }
        inline void setTotalFiles(const qint64 totalFiles) { _totalFiles = totalFiles; }
        inline qint64 totalFiles() const noexcept { return _totalFiles; }
        inline void setCompletedSize(const qint64 completedSize) noexcept { _completedSize = completedSize; }
        inline qint64 completedSize() const noexcept { return _completedSize; }
        inline void setTotalSize(const qint64 totalSize) noexcept { _totalSize = totalSize; }
        inline qint64 totalSize() const noexcept { return _totalSize; }
        inline void setEstimatedRemainingTime(const qint64 estimatedRemainingTime) noexcept {
        ...
        inline void setIsBeingDeleted(const bool isDeletionOnGoing) noexcept { _isBeingDeleted = isDeletionOnGoing; }
