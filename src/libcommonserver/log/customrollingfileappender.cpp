...
void CustomRollingFileAppender::append(const log4cplus::spi::InternalLoggingEvent &event) {
    ...
    const std::string consolPrintableLog = std::to_string(event.getTimestamp().time_since_epoch().count()) + std::string(" - ") +
            SyncPath(event.getFile()).filename().string() + ":" + std::to_string(event.getLine()) + std::string(" - ") + SyncName2Str(event.getMessage());
    try {
