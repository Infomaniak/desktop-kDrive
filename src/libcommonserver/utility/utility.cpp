...
SyncName Utility::normalizedSyncName(const SyncName &name) {
    ...
    if (!str) {
        // Some special characters seem to be not supported, therefore a null pointer is returned if the conversion has
        // failed. e.g.: Linux can sometime send filesystem events with strange charater in the path
        return "";
        // TODO : we should return a boolean value to explicitly say that the conversion has failed. Output value
