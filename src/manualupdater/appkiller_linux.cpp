...
// pkill sends SIGTERM by default (soft) or SIGKILL with -9 (hard).
// The -x flag ensures exact process name matching so the updater itself is never killed.
...
bool killRunningApp(QString &outMessage) {
    ...
    outMessage = QStringLiteral("kDrive could not be stopped. Please force quit kDrive manually (e.g., via System Monitor "
                               "or `kill -9`), then relaunch this tool.");
