...
ExitCode UpdateTreeWorker::step3DeleteDirectory() {
    ...
    for (const auto &deleteOpId : deleteOpsIds) {
        ...
        if (currentNodeIt != _updateTree->nodes().end()) {
            ...
            if (const auto exitCode = searchForParentNode(deleteOp->path(), parentNode); exitCode != ExitCodeOk) {
                return exitCode;
            };

            if (!parentNode) {
                SyncPath newPath;
                if (const auto exitCode = getNewPathAfterMove(deleteOp->path(), newPath); exitCode == ExitCodeDbError) {
