...
bool SyncDb::updateNames(const char *queryId, const SyncName &localName, const SyncName &remoteName) {
    LOG_IF_DB_FAIL(queryBindValue(queryId, 2, localName))
    ...
    LOG_IF_DB_FAIL(queryBindValue(queryId, 3, remoteNormalizedName))
    return true;
}
...
bool SyncDb::insertNode(const DbNode &node, DbNodeId &dbNodeId, bool &constraintError) {
    ...
    LOG_IF_DB_FAIL(queryResetAndClearBindings(queryId))
    LOG_IF_DB_FAIL(queryBindValue(queryId, 1, (node.parentNodeId() ? dbtype(node.parentNodeId().value()) : std::monostate())))
    if (!updateNames(queryId, node.nameLocal(), node.nameRemote())) return false;
    LOG_IF_DB_FAIL(queryBindValue(queryId, 4, (node.nodeIdLocal() ? dbtype(node.nodeIdLocal().value()) : std::monostate())))
    LOG_IF_DB_FAIL(queryBindValue(queryId, 5, (node.nodeIdRemote() ? dbtype(node.nodeIdRemote().value()) : std::monostate())))
    LOG_IF_DB_FAIL(queryBindValue(queryId, 6, (node.created() ? dbtype(node.created().value()) : std::monostate())))
    LOG_IF_DB_FAIL(
            queryBindValue(queryId, 7, (node.lastModifiedLocal() ? dbtype(node.lastModifiedLocal().value()) : std::monostate())))
    LOG_IF_DB_FAIL(queryBindValue(queryId, 8,
                                  (node.lastModifiedRemote() ? dbtype(node.lastModifiedRemote().value()) : std::monostate())))
    LOG_IF_DB_FAIL(queryBindValue(queryId, 9, static_cast<int>(node.type())))
    LOG_IF_DB_FAIL(queryBindValue(queryId, 10, node.size()))
    LOG_IF_DB_FAIL(queryBindValue(queryId, 11, (node.checksum() ? dbtype(node.checksum().value()) : std::monostate())))
    LOG_IF_DB_FAIL(queryBindValue(queryId, 12, static_cast<int>(node.status())))
    LOG_IF_DB_FAIL(queryBindValue(queryId, 13, static_cast<int>(node.syncing())))
...
bool SyncDb::updateNode(const DbNode &node, bool &found) {
    ...
    LOG_IF_DB_FAIL(queryResetAndClearBindings(UPDATE_NODE_REQUEST_ID))
    LOG_IF_DB_FAIL(queryBindValue(UPDATE_NODE_REQUEST_ID, 1,
                                  (node.parentNodeId() ? dbtype(node.parentNodeId().value()) : std::monostate())))
    LOG_IF_DB_FAIL(queryBindValue(UPDATE_NODE_REQUEST_ID, 2, node.nameLocal()))
    LOG_IF_DB_FAIL(queryBindValue(UPDATE_NODE_REQUEST_ID, 3, remoteNormalizedName))
    LOG_IF_DB_FAIL(queryBindValue(UPDATE_NODE_REQUEST_ID, 4,
                                  (node.nodeIdLocal() ? dbtype(node.nodeIdLocal().value()) : std::monostate())))
    LOG_IF_DB_FAIL(queryBindValue(UPDATE_NODE_REQUEST_ID, 5,
                                  (node.nodeIdRemote() ? dbtype(node.nodeIdRemote().value()) : std::monostate())))
    LOG_IF_DB_FAIL(
            queryBindValue(UPDATE_NODE_REQUEST_ID, 6, (node.created() ? dbtype(node.created().value()) : std::monostate())))
    LOG_IF_DB_FAIL(queryBindValue(UPDATE_NODE_REQUEST_ID, 7,
                                  (node.lastModifiedLocal() ? dbtype(node.lastModifiedLocal().value()) : std::monostate())))
    LOG_IF_DB_FAIL(queryBindValue(UPDATE_NODE_REQUEST_ID, 8,
                                  (node.lastModifiedRemote() ? dbtype(node.lastModifiedRemote().value()) : std::monostate())))
    LOG_IF_DB_FAIL(queryBindValue(UPDATE_NODE_REQUEST_ID, 9, static_cast<int>(node.type())))
    LOG_IF_DB_FAIL(queryBindValue(UPDATE_NODE_REQUEST_ID, 10, node.size()))
    LOG_IF_DB_FAIL(
            queryBindValue(UPDATE_NODE_REQUEST_ID, 11, (node.checksum() ? dbtype(node.checksum().value()) : std::monostate())))
    LOG_IF_DB_FAIL(queryBindValue(UPDATE_NODE_REQUEST_ID, 12, static_cast<int>(node.status())))
    LOG_IF_DB_FAIL(queryBindValue(UPDATE_NODE_REQUEST_ID, 13, static_cast<int>(node.syncing())))
    LOG_IF_DB_FAIL(queryBindValue(UPDATE_NODE_REQUEST_ID, 14, node.nodeId()))
...
bool SyncDb::updateNodeStatus(DbNodeId nodeId, SyncFileStatus status, bool &found) {
    ...
    LOG_IF_DB_FAIL(queryResetAndClearBindings(UPDATE_NODE_STATUS_REQUEST_ID));
    LOG_IF_DB_FAIL(queryBindValue(UPDATE_NODE_STATUS_REQUEST_ID, 1, static_cast<int>(status)));
    LOG_IF_DB_FAIL(queryBindValue(UPDATE_NODE_STATUS_REQUEST_ID, 2, nodeId));
...
bool SyncDb::updateNodeLocalName(DbNodeId nodeId, const SyncName &nameLocal, bool &found) {
    ...
    LOG_IF_DB_FAIL(queryResetAndClearBindings(queryId));
    LOG_IF_DB_FAIL(queryBindValue(queryId, 1, nameLocal));
    LOG_IF_DB_FAIL(queryBindValue(queryId, 2, nodeId));
...
bool SyncDb::updateNodesSyncing(bool syncing) {
    ...
    LOG_IF_DB_FAIL(queryResetAndClearBindings(UPDATE_NODES_SYNCING_REQUEST_ID));
    LOG_IF_DB_FAIL(queryBindValue(UPDATE_NODES_SYNCING_REQUEST_ID, 1, static_cast<int>(syncing)));
...
bool SyncDb::updateNodeSyncing(DbNodeId nodeId, bool syncing, bool &found) {
    ...
    LOG_IF_DB_FAIL(queryResetAndClearBindings(UPDATE_NODE_SYNCING_REQUEST_ID));
    LOG_IF_DB_FAIL(queryBindValue(UPDATE_NODE_SYNCING_REQUEST_ID, 1, static_cast<int>(syncing)));
    LOG_IF_DB_FAIL(queryBindValue(UPDATE_NODE_SYNCING_REQUEST_ID, 2, nodeId));
...
bool SyncDb::deleteNode(DbNodeId nodeId, bool &found) {
    ...
    LOG_IF_DB_FAIL(queryResetAndClearBindings(DELETE_NODE_REQUEST_ID));
    LOG_IF_DB_FAIL(queryBindValue(DELETE_NODE_REQUEST_ID, 1, nodeId));
...
bool SyncDb::selectStatus(DbNodeId nodeId, SyncFileStatus &status, bool &found) {
    ...
    LOG_IF_DB_FAIL(queryResetAndClearBindings(SELECT_NODE_STATUS_BY_NODEID_REQUEST_ID));
    LOG_IF_DB_FAIL(queryBindValue(SELECT_NODE_STATUS_BY_NODEID_REQUEST_ID, 1, nodeId));
    ...
    int intResult;
    LOG_IF_DB_FAIL(queryIntValue(SELECT_NODE_STATUS_BY_NODEID_REQUEST_ID, 0, intResult));
...
bool SyncDb::selectSyncing(DbNodeId nodeId, bool &syncing, bool &found) {
    ...
    LOG_IF_DB_FAIL(queryResetAndClearBindings(SELECT_NODE_SYNCING_BY_NODEID_REQUEST_ID));
    LOG_IF_DB_FAIL(queryBindValue(SELECT_NODE_SYNCING_BY_NODEID_REQUEST_ID, 1, nodeId));
    ...
    int intResult;
    LOG_IF_DB_FAIL(queryIntValue(SELECT_NODE_SYNCING_BY_NODEID_REQUEST_ID, 0, intResult));
...
bool SyncDb::node(ReplicaSide side, const NodeId &nodeId, DbNode &dbNode, bool &found) {
    ...
    LOG_IF_DB_FAIL(queryBindValue(id, 1, nodeId));
    ...
    DbNodeId dbNodeId;
    LOG_IF_DB_FAIL(queryInt64Value(id, SELECT_NODE_BY_REPLICAID_DBID, dbNodeId));

    bool ok;
    std::optional<DbNodeId> parentNodeId;
    LOG_IF_DB_FAIL(queryIsNullValue(id, SELECT_NODE_BY_REPLICAID_PARENTID, ok));
    if (ok) {
        parentNodeId = std::nullopt;
    } else {
        DbNodeId dbParentNodeId;
        LOG_IF_DB_FAIL(queryInt64Value(id, SELECT_NODE_BY_REPLICAID_PARENTID, dbParentNodeId));
        parentNodeId = std::make_optional(dbParentNodeId);
    }

    SyncName nameLocal;
    LOG_IF_DB_FAIL(querySyncNameValue(id, SELECT_NODE_BY_REPLICAID_NAMELOCAL, nameLocal));
    SyncName nameDrive;
    LOG_IF_DB_FAIL(querySyncNameValue(id, SELECT_NODE_BY_REPLICAID_NAMEDRIVE, nameDrive));

    std::optional<SyncTime> created;
    LOG_IF_DB_FAIL(queryIsNullValue(id, SELECT_NODE_BY_REPLICAID_CREATED, ok));
    if (ok) {
        created = std::nullopt;
    } else {
        SyncTime timeTmp;
        LOG_IF_DB_FAIL(queryInt64Value(id, SELECT_NODE_BY_REPLICAID_CREATED, timeTmp));
        created = std::make_optional(timeTmp);
    }

    std::optional<SyncTime> lastModified;
    LOG_IF_DB_FAIL(queryIsNullValue(id, SELECT_NODE_BY_REPLICAID_LASTMOD, ok));
    if (ok) {
        lastModified = std::nullopt;
    } else {
        SyncTime timeTmp;
        LOG_IF_DB_FAIL(queryInt64Value(id, SELECT_NODE_BY_REPLICAID_LASTMOD, timeTmp));
        lastModified = std::make_optional(timeTmp);
    }

    int intResult;
    LOG_IF_DB_FAIL(queryIntValue(id, SELECT_NODE_BY_REPLICAID_TYPE, intResult));
    NodeType type = static_cast<NodeType>(intResult);

    int64_t size;
    LOG_IF_DB_FAIL(queryInt64Value(id, SELECT_NODE_BY_REPLICAID_SIZE, size));

    std::optional<std::string> cs;
    LOG_IF_DB_FAIL(queryIsNullValue(id, SELECT_NODE_BY_REPLICAID_CHECKSUM, ok));
    if (ok) {
        cs = std::nullopt;
    } else {
        std::string csTmp;
        LOG_IF_DB_FAIL(queryStringValue(id, SELECT_NODE_BY_REPLICAID_CHECKSUM, csTmp));
        cs = std::make_optional(csTmp);
    }

    LOG_IF_DB_FAIL(queryIntValue(id, SELECT_NODE_BY_REPLICAID_STATUS, intResult));
    SyncFileStatus status = static_cast<SyncFileStatus>(intResult);

    LOG_IF_DB_FAIL(queryIntValue(id, SELECT_NODE_BY_REPLICAID_SYNCING, intResult));
...
bool SyncDb::dbIds(std::unordered_set<DbNodeId> &ids, bool &found) {
    ...
    // Find root node
    LOG_IF_DB_FAIL(queryResetAndClearBindings(SELECT_NODE_BY_PARENTNODEID_ROOT_REQUEST_ID));
    ...
    DbNodeId nodeDbId;
    LOG_IF_DB_FAIL(queryInt64Value(SELECT_NODE_BY_PARENTNODEID_ROOT_REQUEST_ID, 0, nodeDbId));
    ids.insert(nodeDbId);
...
bool SyncDb::path(DbNodeId dbNodeId, SyncPath &localPath, SyncPath &remotePath, bool &found) {
    ...
    LOG_IF_DB_FAIL(queryBindValue(requestId, 1, dbNodeId));
    ...
    for (;;) {
        ...
        SyncName nameLocal;
        LOG_IF_DB_FAIL(querySyncNameValue(requestId, 0, nameLocal));
        SyncName nameDrive;
        LOG_IF_DB_FAIL(querySyncNameValue(requestId, 1, nameDrive)); // Name on the remote drive.
...
bool SyncDb::node(DbNodeId dbNodeId, DbNode &dbNode, bool &found) {
    ...
    LOG_IF_DB_FAIL(queryResetAndClearBindings(id));
    LOG_IF_DB_FAIL(queryBindValue(id, 1, dbNodeId));
    if (!queryNext(id, found)) {
    ...
    bool ok;
    std::optional<DbNodeId> parentNodeId;
    LOG_IF_DB_FAIL(queryIsNullValue(id, SELECT_NODE_BY_NODEID_PARENTID, ok));
    if (ok) {
        parentNodeId = std::nullopt;
    } else {
        DbNodeId dbParentNodeId;
        LOG_IF_DB_FAIL(queryInt64Value(id, SELECT_NODE_BY_NODEID_PARENTID, dbParentNodeId));
        parentNodeId = std::make_optional(dbParentNodeId);
    }

    SyncName nameLocal;
    LOG_IF_DB_FAIL(querySyncNameValue(id, SELECT_NODE_BY_NODEID_NAMELOCAL, nameLocal));

    SyncName nameDrive;
    LOG_IF_DB_FAIL(querySyncNameValue(id, SELECT_NODE_BY_NODEID_NAMEDRIVE, nameDrive));

    NodeId nodeIdLocal;
    LOG_IF_DB_FAIL(queryStringValue(id, SELECT_NODE_BY_NODEID_IDLOCAL, nodeIdLocal));

    NodeId nodeIdDrive;
    LOG_IF_DB_FAIL(queryStringValue(id, SELECT_NODE_BY_NODEID_IDDRIVE, nodeIdDrive));

    std::optional<SyncTime> created;
    LOG_IF_DB_FAIL(queryIsNullValue(id, SELECT_NODE_BY_NODEID_CREATED, ok));
    if (ok) {
        created = std::nullopt;
    } else {
        SyncTime timeTmp;
        LOG_IF_DB_FAIL(queryInt64Value(id, SELECT_NODE_BY_NODEID_CREATED, timeTmp));
        created = std::make_optional(timeTmp);
    }

    std::optional<SyncTime> lastModifiedLocal;
    LOG_IF_DB_FAIL(queryIsNullValue(id, SELECT_NODE_BY_NODEID_LASTMODLOCAL, ok));
    if (ok) {
        lastModifiedLocal = std::nullopt;
    } else {
        SyncTime timeTmp;
        LOG_IF_DB_FAIL(queryInt64Value(id, SELECT_NODE_BY_NODEID_LASTMODLOCAL, timeTmp));
        lastModifiedLocal = std::make_optional(timeTmp);
    }

    std::optional<SyncTime> lastModifiedDrive;
    LOG_IF_DB_FAIL(queryIsNullValue(id, SELECT_NODE_BY_NODEID_LASTMODDRIVE, ok));
    if (ok) {
        lastModifiedDrive = std::nullopt;
    } else {
        SyncTime timeTmp;
        LOG_IF_DB_FAIL(queryInt64Value(id, SELECT_NODE_BY_NODEID_LASTMODDRIVE, timeTmp));
        lastModifiedDrive = std::make_optional(timeTmp);
    }

    int intResult;
    LOG_IF_DB_FAIL(queryIntValue(id, SELECT_NODE_BY_NODEID_TYPE, intResult));
    NodeType type = static_cast<NodeType>(intResult);

    int64_t size;
    LOG_IF_DB_FAIL(queryInt64Value(id, SELECT_NODE_BY_NODEID_SIZE, size));

    std::optional<std::string> cs;
    LOG_IF_DB_FAIL(queryIsNullValue(id, SELECT_NODE_BY_NODEID_CHECKSUM, ok));
    if (ok) {
        cs = std::nullopt;
    } else {
        std::string csTmp;
        LOG_IF_DB_FAIL(queryStringValue(id, SELECT_NODE_BY_NODEID_CHECKSUM, csTmp));
        cs = std::make_optional(csTmp);
    }

    LOG_IF_DB_FAIL(queryIntValue(id, SELECT_NODE_BY_NODEID_STATUS, intResult));
    SyncFileStatus status = static_cast<SyncFileStatus>(intResult);

    LOG_IF_DB_FAIL(queryIntValue(id, SELECT_NODE_BY_NODEID_SYNCING, intResult));
    bool syncing = static_cast<bool>(intResult);
...
bool SyncDb::dbId(ReplicaSide side, const SyncPath &path, DbNodeId &dbNodeId, bool &found) {
    ...
    LOG_IF_DB_FAIL(queryInt64Value(SELECT_NODE_BY_PARENTNODEID_ROOT_REQUEST_ID, 0, dbNodeId));
...
bool SyncDb::clearNodes() {
    ...
    LOG_IF_DB_FAIL(queryResetAndClearBindings(DELETE_NODES_BUT_ROOT_REQUEST_ID));
...
bool SyncDb::id(ReplicaSide side, const SyncPath &path, std::optional<NodeId> &nodeId, bool &found) {
    ...
    if (!names.empty()) {
        ...
        for (std::vector<SyncName>::reverse_iterator nameIt = names.rbegin(); nameIt != names.rend(); ++nameIt) {
            LOG_IF_DB_FAIL(queryResetAndClearBindings(queryId));
            LOG_IF_DB_FAIL(queryBindValue(queryId, 1, nodeDbId));
            LOG_IF_DB_FAIL(queryBindValue(queryId, 2, *nameIt));
            if (!queryNext(queryId, found)) {
                LOGW_WARN(_logger, L"Error getting query result: " << Utility::s2ws(queryId) << L" - parentNodeId=" << nodeDbId
                                                                   << L" and name=" << Utility::formatSyncName(*nameIt));
                return false;
            }
            if (!found) {
                return true;
            }
            LOG_IF_DB_FAIL(queryInt64Value(queryId, 0, nodeDbId));
        }
        bool ok;
        LOG_IF_DB_FAIL(queryIsNullValue(queryId, 1, ok));
        if (ok) {
            nodeId = std::nullopt;
        } else {
            NodeId idTmp;
            LOG_IF_DB_FAIL(queryStringValue(queryId, 1, idTmp));
        ...
        bool ok;
        LOG_IF_DB_FAIL(queryIsNullValue(SELECT_NODE_BY_PARENTNODEID_ROOT_REQUEST_ID, (side == ReplicaSide::Local ? 3 : 4), ok));
        if (ok) {
            nodeId = std::nullopt;
        } else {
            NodeId idTmp;
...
bool SyncDb::type(ReplicaSide side, const NodeId &nodeId, NodeType &type, bool &found) {
    ...
    int result;
    LOG_IF_DB_FAIL(queryIntValue(id, SELECT_NODE_BY_REPLICAID_TYPE, result));
...
bool SyncDb::parent(ReplicaSide side, const NodeId &nodeId, NodeId &parentNodeid, bool &found) {
    ...
    LOG_IF_DB_FAIL(queryInt64Value(id, SELECT_NODE_BY_REPLICAID_PARENTID, parentNodeDbId));
    LOG_IF_DB_FAIL(queryResetAndClearBindings(id));

    LOG_IF_DB_FAIL(queryResetAndClearBindings(SELECT_NODE_BY_NODEID_LITE_ID));
    LOG_IF_DB_FAIL(queryBindValue(SELECT_NODE_BY_NODEID_LITE_ID, 1, parentNodeDbId));
    ...
    LOG_IF_DB_FAIL(queryStringValue(SELECT_NODE_BY_NODEID_LITE_ID, (side == ReplicaSide::Local ? 3 : 4), parentNodeid));
    LOG_IF_DB_FAIL(queryResetAndClearBindings(SELECT_NODE_BY_NODEID_LITE_ID));
...
bool SyncDb::path(ReplicaSide side, const NodeId &nodeId, SyncPath &path, bool &found) {
    ...
    LOG_IF_DB_FAIL(queryResetAndClearBindings(requestId));
    LOG_IF_DB_FAIL(queryBindValue(requestId, 1, nodeId));
    ...
    for (;;) {
        ...
        SyncName name;
        LOG_IF_DB_FAIL(querySyncNameValue(requestId, side == ReplicaSide::Local ? 0 : 1, name));
...
bool SyncDb::name(ReplicaSide side, const NodeId &nodeId, SyncName &name, bool &found) {
    ...
    LOG_IF_DB_FAIL(querySyncNameValue(
...
bool SyncDb::ids(ReplicaSide side, std::vector<NodeId> &ids, bool &found) {
    ...
    std::string id;
    LOG_IF_DB_FAIL(queryStringValue(SELECT_NODE_BY_PARENTNODEID_ROOT_REQUEST_ID, (side == ReplicaSide::Local ? 3 : 4), id));
    ids.push_back(id);
...
bool SyncDb::ancestor(ReplicaSide side, const NodeId &nodeId1, const NodeId &nodeId2, bool &ret, bool &found) {
    ...
    std::string id = (side == ReplicaSide::Local ? SELECT_NODE_BY_NODEIDLOCAL_ID : SELECT_NODE_BY_NODEIDDRIVE_ID);
    LOG_IF_DB_FAIL(queryResetAndClearBindings(id));
    LOG_IF_DB_FAIL(queryBindValue(id, 1, nodeId2));
    if (!queryNext(id, found)) {
        LOG_WARN(_logger, "Error getting query result: "
    ...
    LOG_IF_DB_FAIL(queryIsNullValue(id, SELECT_NODE_BY_REPLICAID_PARENTID, parentNodeDbIdIsNull));
    ...
    do {
        ...
        bool nodeIdIsNull;
        LOG_IF_DB_FAIL(queryIsNullValue(SELECT_NODE_BY_NODEID_LITE_ID, (side == ReplicaSide::Local ? 3 : 4), nodeIdIsNull));
        if (nodeIdIsNull) {
        ...
        NodeId nodeId;
        LOG_IF_DB_FAIL(queryStringValue(SELECT_NODE_BY_NODEID_LITE_ID, (side == ReplicaSide::Local ? 3 : 4), nodeId));
        if (nodeId == nodeId1) {
            ret = true;
            return true;
        }

        LOG_IF_DB_FAIL(queryIsNullValue(SELECT_NODE_BY_NODEID_LITE_ID, 0, parentNodeDbIdIsNull));
        if (!parentNodeDbIdIsNull) {
            LOG_IF_DB_FAIL(queryInt64Value(SELECT_NODE_BY_NODEID_LITE_ID, 0, parentNodeDbId));
        }
...
bool SyncDb::id(ReplicaSide side, DbNodeId dbNodeId, NodeId &nodeId, bool &found) {
    ...
    LOG_IF_DB_FAIL(queryResetAndClearBindings(SELECT_NODE_BY_NODEID_LITE_ID));
    LOG_IF_DB_FAIL(queryBindValue(SELECT_NODE_BY_NODEID_LITE_ID, 1, dbNodeId));
    if (!queryNext(SELECT_NODE_BY_NODEID_LITE_ID, found)) {
        LOG_WARN(_logger,
...
bool SyncDb::updateAllSyncNodes(SyncNodeType type, const std::unordered_set<NodeId> &nodeIdSet) {
    ...
    LOG_IF_DB_FAIL(queryResetAndClearBindings(DELETE_ALL_SYNC_NODE_BY_TYPE_REQUEST_ID));
    LOG_IF_DB_FAIL(queryBindValue(DELETE_ALL_SYNC_NODE_BY_TYPE_REQUEST_ID, 1, toInt(type)));
    ...
    for (const NodeId &nodeId: nodeIdSet) {
        LOG_IF_DB_FAIL(queryResetAndClearBindings(INSERT_SYNC_NODE_REQUEST_ID));
        LOG_IF_DB_FAIL(queryBindValue(INSERT_SYNC_NODE_REQUEST_ID, 1, nodeId));
        LOG_IF_DB_FAIL(queryBindValue(INSERT_SYNC_NODE_REQUEST_ID, 2, toInt(type)));
...
bool SyncDb::selectAllSyncNodes(SyncNodeType type, std::unordered_set<NodeId> &nodeIdSet) {
    ...
    LOG_IF_DB_FAIL(queryResetAndClearBindings(SELECT_ALL_SYNC_NODE_REQUEST_ID));
    LOG_IF_DB_FAIL(queryBindValue(SELECT_ALL_SYNC_NODE_REQUEST_ID, 1, toInt(type)));
    bool found;
    for (;;) {
        ...
        NodeId nodeId;
        LOG_IF_DB_FAIL(queryStringValue(SELECT_ALL_SYNC_NODE_REQUEST_ID, 0, nodeId));
...
bool SyncDb::insertUploadSessionToken(const UploadSessionToken &uploadSessionToken, int64_t &uploadSessionTokenDbId) {
    ...
    LOG_IF_DB_FAIL(queryResetAndClearBindings(INSERT_UPLOAD_SESSION_TOKEN_REQUEST_ID));
    LOG_IF_DB_FAIL(queryBindValue(INSERT_UPLOAD_SESSION_TOKEN_REQUEST_ID, 1, uploadSessionToken.token()));
...
bool SyncDb::deleteUploadSessionTokenByDbId(int64_t dbId, bool &found) {
    ...
    LOG_IF_DB_FAIL(queryResetAndClearBindings(DELETE_UPLOAD_SESSION_TOKEN_BY_DBID_REQUEST_ID));
    LOG_IF_DB_FAIL(queryBindValue(DELETE_UPLOAD_SESSION_TOKEN_BY_DBID_REQUEST_ID, 1, dbId));
...
bool SyncDb::selectAllUploadSessionTokens(std::vector<UploadSessionToken> &uploadSessionTokenList) {
    ...
    LOG_IF_DB_FAIL(queryResetAndClearBindings(SELECT_ALL_UPLOAD_SESSION_TOKEN_REQUEST_ID));
    bool found;
    for (;;) {
        ...
        int id;
        LOG_IF_DB_FAIL(queryIntValue(SELECT_ALL_UPLOAD_SESSION_TOKEN_REQUEST_ID, 0, id));
        std::string token;
        LOG_IF_DB_FAIL(queryStringValue(SELECT_ALL_UPLOAD_SESSION_TOKEN_REQUEST_ID, 1, token));
...
bool SyncDb::deleteAllUploadSessionToken() {
    ...
    LOG_IF_DB_FAIL(queryResetAndClearBindings(DELETE_ALL_UPLOAD_SESSION_TOKEN_REQUEST_ID));
...
bool SyncDb::pushChildIds(ReplicaSide side, DbNodeId parentNodeDbId, std::vector<NodeId> &ids) {
    ...
    while (!dbNodeIdQueue.empty()) {
        ...
        LOG_IF_DB_FAIL(queryResetAndClearBindings(SELECT_NODE_BY_PARENTNODEID_REQUEST_ID));
        LOG_IF_DB_FAIL(queryBindValue(SELECT_NODE_BY_PARENTNODEID_REQUEST_ID, 1, dbNodeId));
        for (;;) {
            bool found;
            ...
            if (!nodeIdIsNull) {
                ...
                LOG_IF_DB_FAIL(queryInt64Value(SELECT_NODE_BY_PARENTNODEID_REQUEST_ID, 0, dbChildNodeId));

                NodeId childNodeId;
                LOG_IF_DB_FAIL(queryStringValue(SELECT_NODE_BY_PARENTNODEID_REQUEST_ID, (side == ReplicaSide::Local ? 3 : 4),
                                                childNodeId));
                ids.push_back(childNodeId);

                int type;
                LOG_IF_DB_FAIL(queryIntValue(SELECT_NODE_BY_PARENTNODEID_REQUEST_ID, 5, type));
...
bool SyncDb::selectAllRenamedNodes(std::vector<DbNode> &dbNodeList, bool onlyColon) {
    ...
    for (;;) {
        ...
        DbNodeId dbNodeId;
        LOG_IF_DB_FAIL(queryInt64Value(requestId, 0, dbNodeId));

        bool ok;
        std::optional<DbNodeId> parentNodeId;
        LOG_IF_DB_FAIL(queryIsNullValue(requestId, 1, ok));
        if (ok) {
            parentNodeId = std::nullopt;
        } else {
            DbNodeId dbParentNodeId;
            LOG_IF_DB_FAIL(queryInt64Value(requestId, 1, dbParentNodeId));
            parentNodeId = std::make_optional(dbParentNodeId);
        }

        SyncName nameLocal;
        LOG_IF_DB_FAIL(querySyncNameValue(requestId, 2, nameLocal));
        SyncName nameDrive;
        LOG_IF_DB_FAIL(querySyncNameValue(requestId, 3, nameDrive));

        std::optional<NodeId> nodeIdLocal;
        LOG_IF_DB_FAIL(queryIsNullValue(requestId, 4, ok));
        if (ok) {
            nodeIdLocal = std::nullopt;
        } else {
            NodeId nodeIdLocalTmp;
            LOG_IF_DB_FAIL(queryStringValue(requestId, 4, nodeIdLocalTmp));
            nodeIdLocal = std::make_optional(nodeIdLocalTmp);
        }

        std::optional<NodeId> nodeIdDrive;
        LOG_IF_DB_FAIL(queryIsNullValue(requestId, 5, ok));
        if (ok) {
            nodeIdDrive = std::nullopt;
        } else {
            NodeId nodeIdDriveTmp;
            LOG_IF_DB_FAIL(queryStringValue(requestId, 5, nodeIdDriveTmp));
            nodeIdDrive = std::make_optional(nodeIdDriveTmp);
        }

        std::optional<SyncTime> created;
        LOG_IF_DB_FAIL(queryIsNullValue(requestId, 6, ok));
        if (ok) {
            created = std::nullopt;
        } else {
            SyncTime timeTmp;
            LOG_IF_DB_FAIL(queryInt64Value(requestId, 6, timeTmp));
            created = std::make_optional(timeTmp);
        }

        std::optional<SyncTime> lastModifiedLocal;
        LOG_IF_DB_FAIL(queryIsNullValue(requestId, 7, ok));
        if (ok) {
            lastModifiedLocal = std::nullopt;
        } else {
            SyncTime timeTmp;
            LOG_IF_DB_FAIL(queryInt64Value(requestId, 7, timeTmp));
            lastModifiedLocal = std::make_optional(timeTmp);
        }

        std::optional<SyncTime> lastModifiedDrive;
        LOG_IF_DB_FAIL(queryIsNullValue(requestId, 8, ok));
        if (ok) {
            lastModifiedDrive = std::nullopt;
        } else {
            SyncTime timeTmp;
            LOG_IF_DB_FAIL(queryInt64Value(requestId, 8, timeTmp));
            lastModifiedDrive = std::make_optional(timeTmp);
        }
        ...
        int64_t size;
        LOG_IF_DB_FAIL(queryInt64Value(requestId, 10, size));

        std::optional<std::string> cs;
        LOG_IF_DB_FAIL(queryIsNullValue(requestId, 11, ok));
        if (ok) {
            cs = std::nullopt;
        } else {
            std::string csTmp;
            LOG_IF_DB_FAIL(queryStringValue(requestId, 9, csTmp));
            cs = std::make_optional(csTmp);
        }
        ...
        LOG_IF_DB_FAIL(queryIntValue(requestId, 11, intResult));
        bool syncing = static_cast<bool>(intResult);
...
bool SyncDb::deleteNodesWithNullParentNodeId() {
    ...
    LOG_IF_DB_FAIL(queryResetAndClearBindings(DELETE_NODES_WITH_NULL_PARENTNODEID_REQUEST_ID));
...
bool SyncDb::pushChildDbIds(DbNodeId parentNodeDbId, std::unordered_set<DbNodeId> &ids) {
    ...
    while (!dbNodeIdQueue.empty()) {
        ...
        for (;;) {
            ...
            bool nodeIdIsNull;
            LOG_IF_DB_FAIL(queryIsNullValue(SELECT_NODE_BY_PARENTNODEID_REQUEST_ID, 0, nodeIdIsNull));
            if (!nodeIdIsNull) {
                // The node exists in the snapshot
                DbNodeId dbChildNodeId;
                LOG_IF_DB_FAIL(queryInt64Value(SELECT_NODE_BY_PARENTNODEID_REQUEST_ID, 0, dbChildNodeId));
...
bool SyncDb::selectNamesWithDistinctEncodings(NamedNodeMap &namedNodeMap) {
    ...
    for (;;) {
        ...
        SyncName nameLocal;
        LOG_IF_DB_FAIL(querySyncNameValue(requestId, 1, nameLocal));
        ...
        SyncName nameDrive;
        LOG_IF_DB_FAIL(querySyncNameValue(requestId, 2, nameDrive));

        NodeId nodeIdLocal;
        LOG_IF_DB_FAIL(queryStringValue(requestId, 3, nodeIdLocal));
