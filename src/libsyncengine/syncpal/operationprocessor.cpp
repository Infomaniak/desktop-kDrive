...
std::shared_ptr<Node> OperationProcessor::correspondingNodeInOtherTree(const std::shared_ptr<Node> node) {
    ...
    if (!dbNodeId && node->id()) {
        ...
        bool found = false;
        if (!_syncPal->syncDb()->cache().dbId(node->side(), *node->id(), tmpDbNodeId, found)) {
...
std::shared_ptr<Node> OperationProcessor::findCorrespondingNodeFromPath(const std::shared_ptr<Node> node) {
    ...
    while (parentNode != nullptr && !found) {
        ...
        if (!_syncPal->syncDb()->cache().dbId(node->side(), *parentNode->id(), parentDbNodeId, found)) {
    ...
    NodeId parentNodeId;
    if (!_syncPal->syncDb()->cache().id(otherSide(node->side()), parentDbNodeId, parentNodeId, found)) {
...
std::shared_ptr<Node> OperationProcessor::correspondingNodeDirect(const std::shared_ptr<Node> node) {
    ...
    if (!_syncPal->syncDb()->cache().id(otherSide(node->side()), *node->idb(), correspondingId, found)) {
