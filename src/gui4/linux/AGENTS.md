...
- `communicationlayer/certreader.*`: read-only accessor for the public TLS certificate stored in the OS keychain. Uses
  the shared `package` / `service` constants from `libcommon/comm.h` (same slots as `KeyChainStorage`) and the
  `certKeychainKey` key; returns `false` (not an error) when the entry is not present yet, so the IPC client can retry
  during startup.
...
- Transport is loopback TCP + JSON envelope shared with `libcommon/comm.h` / `libcommon/commjson.h`, wrapped in TLS via
  ...
  (`CertReader` loads it at startup) and verifies the peer name against `kDrive-localhost`; SSL errors are logged and
  never ignored. The shared keychain `package` / `service` constants live in `libcommon/comm.h` and must stay in sync
  with `KeyChainStorage` on the server side.
