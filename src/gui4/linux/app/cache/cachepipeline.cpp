...
CachePipeline::CachePipeline(CommService &commService, AppCache &appCache, QObject *const parent) :
    ...
    (void) connect(&_commService, &CommService::errorAdded, &_appCache, &AppCache::upsertSyncError, Qt::UniqueConnection);
    (void) connect(&_commService, &CommService::errorRemoved, &_appCache, &AppCache::removeSyncError, Qt::UniqueConnection);
}
