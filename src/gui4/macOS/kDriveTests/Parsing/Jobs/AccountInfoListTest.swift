...
@Suite("AccountInfoList Job Parsing Test")
struct AccountInfoListTest {
    ...
    var validJobCallbackData: Data {
        ...
        guard let url = bundle.url(forResource: "ACCOUNT_INFOLIST", withExtension: "json") else {
    ...
    @Test("Successfully parses a valid ACCOUNT_INFOLIST.json")
    func parseValidJobCallback() throws {
        ...
        #expect(firstAccount?.userDbId == 1)
        #expect(firstAccount?.name == "Infomaniak Group")
    }
}
