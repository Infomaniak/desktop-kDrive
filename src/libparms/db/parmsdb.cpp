...
#define CHECK_COLUMN_EXISTENCE_REQUEST "SELECT COUNT(*) AS CNTREC FROM pragma_table_info('?1') WHERE name='?2';"
    ...
    bool ParmsDb::insertUserTemplateNormalizations(const std::string &fromVersion) {
        ...
        if (!createAndPrepareRequest(SELECT_ALL_EXCLUSION_TEMPLATE_BY_DEF_REQUEST_ID,
        ...
        for (const auto &userTemplate: dbUserExclusionTemplates) {
            ...
            for (const auto &normalizedName: normalizedTemplateNames) {
                ...
                if (const auto &[_, successfulInsertion] = dbUserExclusionUniqueStrings.emplace(normalizedStr);
                    successfulInsertion) {
        ...
        if (!createAndPrepareRequest(DELETE_ALL_EXCLUSION_TEMPLATE_BY_DEF_REQUEST_ID,
    ...
    bool ParmsDb::prepare() {
        ...
        if (!createAndPrepareRequest(UPDATE_SYNC_HASFULLYCOMPLETED_REQUEST_ID, UPDATE_SYNC_HASFULLYCOMPLETED_REQUEST))
            return false;
        ...
        if (!createAndPrepareRequest(DELETE_ALL_EXCLUSION_TEMPLATE_BY_DEF_REQUEST_ID,
                                     DELETE_ALL_EXCLUSION_TEMPLATE_BY_DEF_REQUEST))
            return false;
        if (!createAndPrepareRequest(SELECT_ALL_EXCLUSION_TEMPLATE_REQUEST_ID, SELECT_ALL_EXCLUSION_TEMPLATE_REQUEST))
            return false;
        if (!createAndPrepareRequest(SELECT_ALL_EXCLUSION_TEMPLATE_BY_DEF_REQUEST_ID,
                                     SELECT_ALL_EXCLUSION_TEMPLATE_BY_DEF_REQUEST))
            return false;
        ...
        if (!createAndPrepareRequest(SELECT_FILTERED_CONFLICTS_BY_SYNCDBID_REQUEST_ID,
            ...
            return false;
        // Migration old selectivesync table
        if (!createAndPrepareRequest(INSERT_MIGRATION_SELECTIVESYNC_REQUEST_ID, INSERT_MIGRATION_SELECTIVESYNC_REQUEST))
            return false;
    ...
    bool ParmsDb::accountFromUserDbIdAndAccountId(const UserDbId userDbId, const AccountId accountId, Account &account,
                                                  bool &found) {
        found = false;
    ...
    bool ParmsDb::selectAllDrives(const AccountDbId accountDbId, std::vector<Drive> &driveList) {
        ...
        for (;;) {
            ...
            (void) driveList.emplace_back(Drive(id, driveId, accountDbId, driveName, size, color,
                                                static_cast<bool>(notifications), static_cast<bool>(admin)));
        }
        LOG_IF_FAIL(queryResetAndClearBindings(SELECT_ALL_DRIVES_BY_ACCOUNT_REQUEST_ID));
    ...
    bool ParmsDb::selectAllExclusionTemplates(const bool defaultTemplates,
    ...
    bool ParmsDb::selectErrorByNodeInfo(SyncDbId syncDbId, const std::optional<NodeId> &localNodeId,
        ...
        for (;;) {
            ...
            errs.push_back(Error(dbId, time, ErrorLevel::Node, functionName, syncDbId, workerName,
                                 static_cast<ExitCode>(exitCode), static_cast<ExitCause>(exitCause),
                                 static_cast<NodeId>(localNodeId), static_cast<NodeId>(remoteNodeId),
                                 static_cast<NodeType>(nodeType), static_cast<SyncPath>(path),
                                 static_cast<ConflictType>(conflictType), static_cast<InconsistencyType>(inconsistencyType),
                                 static_cast<CancelType>(cancelType), static_cast<SyncPath>(destinationPath)));
        }
        LOG_IF_FAIL(queryResetAndClearBindings(SELECT_ERRORS_BY_NODE_INFOS_ID));
