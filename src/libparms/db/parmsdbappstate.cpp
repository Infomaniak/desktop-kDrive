...
bool ParmsDb::insertAppState(AppStateKey key, const std::string &value, const bool updateOnlyIfEmpty /*= false*/) {
    ...
    LOG_IF_DB_FAIL(queryResetAndClearBindings(SELECT_APP_STATE_REQUEST_ID))
    LOG_IF_DB_FAIL(queryBindValue(SELECT_APP_STATE_REQUEST_ID, 1, static_cast<int>(key)))
    bool found = false;
    ...
    if (found) {
        LOG_IF_DB_FAIL(queryStringValue(SELECT_APP_STATE_REQUEST_ID, 0, existingValue))
    }
    LOG_IF_DB_FAIL(queryResetAndClearBindings(SELECT_APP_STATE_REQUEST_ID))
    ...
    if (!found || (updateOnlyIfEmpty && existingValue.empty())) {
        ...
        LOG_IF_DB_FAIL(queryBindValue(requestId, 1, static_cast<int>(key)))
        LOG_IF_DB_FAIL(queryBindValue(requestId, 2, valueStr))
        int errId = 0;
        std::string error;
...
bool ParmsDb::selectAppState(AppStateKey key, AppStateValue &value, bool &found) {
    ...
    LOG_IF_DB_FAIL(queryStringValue(SELECT_APP_STATE_REQUEST_ID, 0, valueStr))
    LOG_IF_DB_FAIL(queryResetAndClearBindings(SELECT_APP_STATE_REQUEST_ID))
...
bool ParmsDb::updateAppState(AppStateKey key, const AppStateValue &value, bool &found) {
    ...
    if (found) {
        std::string error;
        LOG_IF_DB_FAIL(queryResetAndClearBindings(UPDATE_APP_STATE_REQUEST_ID))
        LOG_IF_DB_FAIL(queryBindValue(UPDATE_APP_STATE_REQUEST_ID, 1, static_cast<int>(key)))
        LOG_IF_DB_FAIL(queryBindValue(UPDATE_APP_STATE_REQUEST_ID, 2, valueStr))
