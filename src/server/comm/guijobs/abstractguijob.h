...
class AbstractGuiJob : public AbstractJob {
    public:
        ...
        void setCommManager(const std::shared_ptr<CommManager> &commManager) { _commManager = commManager; }

        void setChannels(const std::list<std::shared_ptr<AbstractCommChannel>> &channels) { _channels = channels; }
        auto channels() const { return _channels; }

        int id() const { return _signalId; }
        RequestNum requestNum() const { return _requestNum; }
        SignalNum signalNum() const { return _signalNum; }
        GuiJobType type() const { return _type; }
...
template<class C>
const std::function<Poco::Dynamic::Var(const C &)> info2DynamicVar = [](const C &value) {
...
template<class C>
const std::function<C(const Poco::Dynamic::Var &)> dynamicVar2Struct = [](const Poco::Dynamic::Var &value) {
