...
bool GuiCommChannel::containsCompleteMessage(const CommString &message, size_t &endIndex) const {
    endIndex = 0;
    if (message.empty()) {
        return false;
    }
    ...
    if ((message[0] != '{' && message[0] != '[')) {
        LOGW_WARN(Log::instance()->getLogger(), L"Invalid JSON: " << CommonUtility::commString2WStr(message));
        return false;
    }
