...
PassiveUpdateNotifier::PassiveUpdateNotifier(const QUrl &url) : KDCUpdater(url) {
    ...
    _runningAppVersion = OldUtility::versionOfInstalledBinary();
}
...
void PassiveUpdateNotifier::backgroundCheckForUpdate() {
    if (OldUtility::isLinux()) {
        ...
        const QByteArray fsVersion = OldUtility::versionOfInstalledBinary();
        if (!(fsVersion.isEmpty() || _runningAppVersion.isEmpty()) && fsVersion != _runningAppVersion) {
            emit requestRestart();
        }
    }

    KDCUpdater::backgroundCheckForUpdate();
}
