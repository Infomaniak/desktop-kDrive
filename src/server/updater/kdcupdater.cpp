...
UpdaterScheduler::UpdaterScheduler(QObject *parent) : QObject(parent) {
    ...
    if (const KDCUpdater *updater = qobject_cast<const KDCUpdater *>(UpdaterServer::instance())) {
...
QString KDCUpdater::statusString() const {
    ...
    switch (downloadState()) {
        ...
        case DownloadFailed:
            return tr(R"(Could not download update.<br>Please download it from <a style="%1" href="%2">here</a>.)")
                .arg(CommonUtility::linkStyle, _updateInfo.web());
        case DownloadTimedOut:
            return tr("Could not check for new updates.");
        case UpdateOnlyAvailableThroughSystem:
            return tr(R"(An update is available: %1.<br>Please download it from <a style="%2" href="%3">here</a>.)")
...
void KDCUpdater::slotStartWindowsInstaller() {
    QString updateFile = QString::fromStdString(ParametersCache::instance()->parameters().updateFileAvailable());
