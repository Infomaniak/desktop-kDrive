...
KDContextMenu::KDContextMenu(void) : m_cRef(1) {
    InterlockedIncrement(&g_cDllRef);
}

KDContextMenu::~KDContextMenu(void) {
...
IFACEMETHODIMP_(ULONG) KDContextMenu::AddRef() {
    return InterlockedIncrement(&m_cRef);
}

// Decrease the reference count for an interface on an object.
IFACEMETHODIMP_(ULONG) KDContextMenu::Release() {
...
IFACEMETHODIMP KDContextMenu::Initialize(LPCITEMIDLIST pidlFolder, LPDATAOBJECT pDataObj, HKEY hKeyProgID) {
...
IFACEMETHODIMP KDContextMenu::QueryContextMenu(HMENU hMenu, UINT indexMenu, UINT idCmdFirst, UINT idCmdLast, UINT uFlags) {
    // If uFlags include CMF_DEFAULTONLY then we should not do anything.
    if (CMF_DEFAULTONLY & uFlags) {
        return MAKE_HRESULT(SEVERITY_SUCCESS, 0, USHORT(0));
    }

    m_info = KDClientInterface::FetchInfo(m_selectedFiles);
    ...
    HMENU hSubmenu = CreateMenu();
    {
        ...
        if (!InsertMenuItem(hMenu, indexMenu++, TRUE, &mii)) return HRESULT_FROM_WIN32(GetLastError());
    }
    InsertSeperator(hMenu, indexMenu++);
    ...
    for (auto &item : m_info.menuItems) {
        ...
        if (!InsertMenuItem(hSubmenu, indexSubMenu, true, &mii)) return HRESULT_FROM_WIN32(GetLastError());
        indexSubMenu++;
    }
...
IFACEMETHODIMP KDContextMenu::InvokeCommand(LPCMINVOKECOMMANDINFO pici) {
    std::wstring command;
    ...
    KDClientInterface::SendRequest(command.data(), m_selectedFiles);
    return S_OK;
}

IFACEMETHODIMP KDContextMenu::GetCommandString(UINT_PTR idCommand, UINT uFlags, UINT *pwReserved, LPSTR pszName, UINT cchMax) {
    if (idCommand < m_info.menuItems.size() && uFlags == GCS_VERBW) {
        return StringCchCopyW(reinterpret_cast<PWSTR>(pszName), cchMax, m_info.menuItems[idCommand].command.data());
    }
    return E_INVALIDARG;
}
