...
function clean_app_directory() {
  ...
  # Clean translations - keep only de, es, fr, it, sv, pt, pl, en
  ...
  for prefix in qt qtbase qtconnectivity qtdeclarative qtlocation qtmultimedia qtserialport qtwebsockets; do
    # Remove all except de, es, fr, it, sv, pt, pl, en
    find . -mindepth 1 -maxdepth 1 \( -type f -o -type l \) -name "${prefix}_*.qm" \
      ! -name "*_de.qm" ! -name "*_es.qm" ! -name "*_fr.qm" ! -name "*_it.qm" ! -name "*_sv.qm" ! -name "*_pt.qm" ! -name "*_pl.qm" ! -name "*_en.qm" -delete
  done
