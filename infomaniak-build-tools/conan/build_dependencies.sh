...
if [ "$platform" = "darwin" ]; then
  ...
  conan create "$conan_recipes_folder/openssl-universal/all/" --build=missing --profile:all="$conan_profile" -r="$local_recipe_remote_name" -r=conancenter
fi
...
conan create "$conan_recipes_folder/qt/all/" --version="$qt_version" --build=missing $architecture -r=$local_recipe_remote_name -r=conancenter

log "Creating sentry package..."
conan create "$conan_recipes_folder/sentry/all/" --build=missing $architecture -r=$local_recipe_remote_name -r=conancenter
