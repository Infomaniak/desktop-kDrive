...
#>
param(
    [Parameter(Mandatory=$true)][switch]$BuildDir,
...
if ($runScript -like '*.ps1') {
    ...
    & $runScript
} elseif ($runScript -like '*.bat') {
    Log "Executing batch script via cmd.exe: $runScript"
    cmd /c $runScript
} else {
    Err "Unsupported script type: $runScript"
}
